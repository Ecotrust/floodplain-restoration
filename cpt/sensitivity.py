...
if __name__ == "__main__":
    ...
    with open('sa/params.txt', 'r') as fh:
        params = [x.split()[0] for x in fh.readlines()]

    with open('sa/results.txt', 'w') as ofh:
        with open('sa/samples.txt', 'r') as fh:
            for i, sample in enumerate(fh.readlines()):
                if i % 50 == 0:
                    print i
                sample_list = [float(x) for x in sample.split()]
                sample_dict = dict(zip(params, sample_list))
                res = query_cpt(user_data = sample_dict)
                ofh.write("%s\n" % res)
